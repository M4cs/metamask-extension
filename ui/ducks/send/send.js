...
const computeEstimatedGasLimit = createAsyncThunk(
...
export const initialState = {
  ...
  gas: {
    // which gas estimation mode is being used
    mode: GAS_INPUT_MODES.BASIC,
...
const slice = createSlice({
  ...
  reducers: {
    ...
    useBasicGasFields: (state) => {
      // Show the basic gas estimation buttons in the send page
      state.gas.mode = GAS_INPUT_MODES.BASIC;
    },
    useInlineGasFields: (state) => {
      // Show the inline gas price/limit fields in the send page
      state.gas.mode = GAS_INPUT_MODES.INLINE;
    },
    useCustomGasFee: (state) => {
      // Show the gas fees set in the custom gas modal (state.gas.customData)
      state.gas.mode = GAS_INPUT_MODES.CUSTOM;
    },
    ...
    validateSendState: (state) => {
      switch (true) {
        ...
        // 5. State is invalid if gasLimit is less than the minimumGasLimit
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(initializeSendState.pending, (state) => {
        ...
        // prevent the form from being submitted to allow for related state
