...
describe('Send Slice', () => {
  describe('Reducers', () => {
    ...
    describe('useBasicGasFields', () => {
      it('should', () => {
        const action = {
          type: 'send/useBasicGasFields',
        };

        const result = sendReducer(initialState, action);

        expect(result.gas.mode).toStrictEqual(GAS_INPUT_MODES.BASIC);
      });
    });
    ...
    describe('useCustomGasFee', () => {
      it('should', () => {
        const action = {
          type: 'send/useCustomGasFee',
        };

        const result = sendReducer(initialState, action);

        expect(result.gas.mode).toStrictEqual(GAS_INPUT_MODES.CUSTOM);
      });
    });
  ...
  describe('extraReducers/externalReducers', () => {
    ...
    describe('BASIC_GAS_ESTIMATE_STATUS', () => {
      ...
      it('should invalidate the send status when status is FAILED and use INLINE gas input mode', () => {
        ...
        expect(result.status).not.toStrictEqual(validSendStatusState.status);
        expect(result.gas.mode).toStrictEqual(GAS_INPUT_MODES.INLINE);
      });
    });

    describe('SET_ESTIMATE_SOURCE', () => {
      it('should use inline fields when source is ETHGASPRICE', () => {
        const action = {
          type: 'metamask/gas/SET_ESTIMATE_SOURCE',
          value: GAS_SOURCE.ETHGASPRICE,
        };
      ...
      it('should use basic gas buttons when source is METASWAPS', () => {
        const mockState = {
          ...initialState,
          gas: {
            ...initialState.gas,
            mode: GAS_INPUT_MODES.INLINE,
          },
        };

        const action = {
          type: 'metamask/gas/SET_ESTIMATE_SOURCE',
          value: GAS_SOURCE.METASWAPS,
        };

        const result = sendReducer(mockState, action);
